...
from ..utils.locale import get_profile_language_hint, normalize_locale
...
class OasisProfileGenerator:
    ...
    def __init__(
        ...
        self.language_hint = get_profile_language_hint(self.locale)
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """获取系统提示词"""
        base_prompt = f"你是社交媒体用户画像生成专家。生成详细、真实的人设用于舆论模拟,最大程度还原已有现实情况。必须返回有效的JSON格式，所有字符串值不能包含未转义的换行符。{self.language_hint}"
        return base_prompt
...
6. country: 国家/地区（使用目标语言描述，必要时可保留英文专名）
7. profession: 职业
