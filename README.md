...
| **Python** | 3.11 أو 3.12 | تشغيل الواجهة الخلفية | `python --version` |
...
أوامر إضافية مفيدة أثناء التطوير:

```bash
npm run test:backend   # تشغيل اختبارات الخلفية
npm run smoke:backend  # فحص إقلاع Flask وواجهة /health
...
# سيقوم uv باستخدام Python 3.12 كما هو محدد في المشروع
```
...
# 2) بناء الصورة وتشغيلها محلياً
docker compose up --build -d
```

يقرأ Docker ملف `.env` من جذر المشروع ويستخدم المنافذ `3000` للواجهة الأمامية و`5001` للواجهة الخلفية.
الملف `docker-compose.yml` في هذه النسخة يبني الصورة محلياً من الكود الحالي بدلاً من الاعتماد على صورة خارجية، وهذا يجعل نتائجك متطابقة مع الفرع العربي نفسه.
...
## 📦 إنشاء حزمة نظيفة للنشر أو التسليم

لإنتاج ملف ZIP نظيف بدون `node_modules` أو `.venv` أو مخرجات البناء المحلية:

```bash
npm run package:release
```

سيتم إنشاء الملف في `release/mirofish-arabic-release.zip`.
